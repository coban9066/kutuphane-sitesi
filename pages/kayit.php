...
if ($result->num_rows > 0) {
    ...
    echo "<script>
            ...
            window.location.href = 'login-register.php'; // Kullanıcıyı login-register.php sayfasına yönlendiriyoruz
          </script>";
} else {
...
if (strlen($ad) < 2) {
    echo "<script>alert('Ad en az 2 harften oluşmalıdır.');  window.location.href = 'login-register.php';</script>";
    exit;
}

if (strlen($soyad) < 2) {
    echo "<script>alert('Soyad en az 2 harften oluşmalıdır.');window.location.href = 'login-register.php';</script>";
    exit;
}

// Şifre doğrulaması
if (strlen($sifre) < 8) {
    echo "<script>alert('Şifre en az 8 harften oluşmalıdır.');window.location.href = 'login-register.php';</script>";
    exit;
}
...
$result = $stmt->get_result();
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Kayıt başarılı!'); window.location.href = 'kirala.php';</script>";
    } else {
